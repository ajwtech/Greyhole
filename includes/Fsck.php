...
define('EXECUTE_IMMEDIATELY', TRUE);

class Fsck {
    const OPTION_EMAIL = 'email';
    const OPTION_IF_CONF_CHANGED = 'if-conf-changed';
    const OPTION_CHECKSUMS = 'checksums';
    const OPTION_SKIP_METASTORE = 'skip-metastore';
    const OPTION_ORPHANED = 'orphaned';
    const OPTION_DU = 'du';
    const OPTION_DEL_ORPHANED_METADATA = 'del-orphaned-metadata';
    ...
    public static function scheduleForAllShares($fsck_options = array()) {
        $ids = array();
        // Create one fsck run for each share. That way, if a fsck is interrupted, only the one for that share will be restarted, not all of them.
        foreach (SharesConfig::getShares() as $share_name => $share_options) {
            $ids[] = static::scheduleForDir($share_options[CONFIG_LANDING_ZONE], $fsck_options);
        }
        if (!empty($ids)) {
            // Group together all the just-created fsck runs, to be able to send just one report for all of them.
            $q = "UPDATE fsck_runs SET group_id = :group WHERE id IN (" . implode(',', $ids) . ")";
            DB::execute($q, $ids[0]);
        }
    }

    public static function scheduleForDir($dir, $fsck_options = array(), $executeImmediately = FALSE) {
        // Save the details (dir and options) in `fsck_runs` table
        $q = "INSERT INTO fsck_runs SET scheduled_at = NOW(), dir = :dir, options = :options";
        $params = array(
            'dir' => $dir,
            'options' => empty($fsck_options) ? NULL : implode('|', $fsck_options)
        );
        $fsck_run_id = DB::insert($q, $params);

        if (!$executeImmediately) {
            // Scheduling the fsck using the `tasks` table, in order to execute it at the right time
            DB::insert("INSERT INTO tasks SET action = 'fsck', share = '', complete = 'yes', additional_info = :fsck_run_id", $fsck_run_id);
        }

        return $fsck_run_id;
    }

    public static function runForLZ($dir, $share_name, $fsck_options = array()) {
        $fsck_run_id = Fsck::scheduleForDir($dir, $fsck_options, EXECUTE_IMMEDIATELY);
        $db_infos = DB::getFirst("SELECT * FROM fsck_runs WHERE id = :id", $fsck_run_id);
        $fsck_run = new Fsck($db_infos);
        $fsck_run->started();
        if ($fsck_run->hasOption(Fsck::OPTION_DU)) {
            static::resetDUStats($share_name);
        }
        $fsck_run->fsck($dir, $share_name);
        $fsck_run->ended();
    }
    ...
    public function run() {
        ...
        if ($this->hasOption(Fsck::OPTION_IF_CONF_CHANGED)) {
        ...
        if (!empty($this->db_infos->task_id)) {
            $task_id = $this->db_infos->task_id;
            $query = "UPDATE tasks SET complete = 'yes' WHERE action = 'fsck_file' AND complete = 'idle' AND id < $task_id $where_clause";
            DB::execute($query, $params);
            $num_updated_rows = DB::getFirstValue("SELECT COUNT(*) AS num_updated_rows FROM tasks WHERE action = 'fsck_file' AND complete = 'yes' AND id < $task_id $where_clause", $params);
            if ($num_updated_rows > 0) {
                // Updated some fsck_file to complete; let's just return here, to allow them to be executed first.
                Log::info("Will execute all ($num_updated_rows) pending fsck_file operations for $fsck_what_dir before running this fsck (task ID $task_id).");
                return;
            }
        }
        ...
        if ($this->hasOption(Fsck::OPTION_CHECKSUMS)) {
        ...
        if ($share_options === FALSE) {
            if ($storage_volume !== FALSE) {
                ...
                foreach (SharesConfig::getShares() as $share_name => $share_options) {
                    $this->fsck("$storage_volume/$share_name", $share_name, $storage_volume);
                }
            } else {
            ...
            if ($fsck_type == FSCK_TYPE_SHARE) {
                ...
                if ($this->hasOption(Fsck::OPTION_DU)) {
                    static::resetDUStats($share, $subdir);
                }
            }
            ...
            if ($fsck_type != FSCK_TYPE_METASTORE) {
                $this->fsck($fscked_dir, $share, $storage_volume);
            }

            Log::debug("  Scan metadata stores? " . ($this->hasOption(Fsck::OPTION_SKIP_METASTORE) ? 'No' : 'Yes'));
            if ($this->hasOption(Fsck::OPTION_SKIP_METASTORE) === FALSE) {
                if ($fsck_type == FSCK_TYPE_METASTORE) {
                    ...
                    $this->fsckMetastore($metastore, $subdir, $share);
                } else {
            ...
            if ($fsck_type != FSCK_TYPE_STORAGE_POOL_DRIVE && $this->hasOption(Fsck::OPTION_ORPHANED)) {
                $subdir = "/$share" . str_replace($share_options[CONFIG_LANDING_ZONE], '', $fscked_dir);
                Log::debug("Starting orphans search for $subdir");
                foreach (Config::storagePoolDrives() as $sp_drive) {
                    if (StoragePool::is_pool_drive($sp_drive)) {
                        $this->fsck("$sp_drive/$subdir", $share, $sp_drive);
                    }
                }
            }
        }
        ...
        if ($this->hasOption(Fsck::OPTION_EMAIL)) {
            // @TODO Check if all fsck runs of the group are completed
            // Email report for fsck
            $email_to = Config::get(CONFIG_EMAIL_TO);
            Log::debug("Sending fsck report to $email_to");
            mail($email_to, 'fsck of Greyhole shares on ' . exec('hostname'), $this->getReport());
        }
    }

    public function __construct($db_infos = array()) {
        if (is_array($db_infos)) {
            $this->db_infos = new stdClass();
            $this->db_infos->options = implode('|', $db_infos);
        } else {
    ...
    public function getId() {
        if (!empty($this->db_infos->id)) {
            return (int) $this->db_infos;
        }
        return FALSE;
    }

    public function started() {
        global $current_fsck_run;
        $current_fsck_run = $this;
        if (!empty($this->db_infos->id)) {
            DB::execute("UPDATE fsck_runs SET started_at = NOW() WHERE id = :id", $this->db_infos->id);
        }
    }

    public function ended() {
        global $current_fsck_run;
        $current_fsck_run = NULL;
        if (!empty($this->db_infos->id)) {
            DB::execute("UPDATE fsck_runs SET ended_at = NOW() WHERE id = :id", $this->db_infos->id);
            $this->db_infos = DB::getFirst("SELECT * FROM fsck_runs WHERE id = :id", $this->db_infos->id);
        }
    }

    public function getDir() {
        return $this->db_infos->dir;
    }

    public function hasOption($option) {
        return string_contains($this->db_infos->options, $option);
    }

    public function fsck($path, $share, $storage_path = FALSE) {
        $path = clean_dir($path);
        Log::debug("Entering $path");
        $this->incrStats('landing_zone', 'num_dirs');
        ...
        while (($filename = readdir($handle)) !== FALSE) {
            if ($filename != '.' && $filename != '..') {
                ...
                if ($file_type == 'dir') {
                    $this->fsck($full_path, $share, $storage_path);
                } else {
                    $this->fsckFile($path, $filename, $file_type, 'landing_zone', $share, $storage_path);
    ...
    public function fsckFile($path, $filename, $file_type, $source, $share, $storage_path = FALSE) {
        ...
        if ($source == 'metastore') {
            $this->incrStats('metastore', 'num_files');
        }
        if ($file_type !== FALSE) {
            $this->incrStats('landing_zone', 'num_files');
        }
        if ($file_type == 'file') {
            if ($storage_path === FALSE) {
                ...
                if ($file_type === FALSE) {
                    if (!Log::actionIs(ACTION_FSCK_FILE)) {
                        ...
                        $params = array(
                            ...
                            'fsck_options' => $this->db_infos->options,
                        );
            ...
            foreach ($file_copies_inodes as $key => $real_full_path) {
                ...
                if ($file_size != $expected_file_size) {
                    ...
                    if ($file_size === FALSE) {
                        ...
                        $this->log('wrong_file_size', json_encode(array(
                            'file_path' => clean_dir($real_full_path),
                            'file_size' => $file_size,
        ...
        if (count($file_copies_inodes) == $num_copies_required) {
            ...
            if (!$found_linked_metafile || ($file_type != 'link' && $storage_path === FALSE)) {
                ...
                if (!$found_linked_metafile) {
                    ...
                    $this->incrStats('symlink_target_moved');
                } else {
                    // ... it was missing
                ...
                foreach ($file_metafiles as $key => $metafile) {
                    if ($metafile->is_linked) {
                        static::updateSymlink($metafile->path, "$landing_zone/$file_path/$filename", $share, $file_path, $filename);
                        break;
                    }
                }
            ...
            if ($source == 'metastore' || get_metafile_data_filename($share, $file_path, $filename) !== FALSE) {
                $this->log('no_copies_found_files', clean_dir("$share/$file_path/$filename"));
            }
            ...
            if ($this->hasOption(Fsck::OPTION_DEL_ORPHANED_METADATA)) {
            ...
            if (count($file_copies_inodes) > $num_copies_required) {
                ...
                $this->incrStats('too_many_copies');
    ...
    public function fsckMetastore($root, $path, $share) {
        if (!is_dir("$root$path")) {
            // Try NFC form [http://en.wikipedia.org/wiki/Unicode_equivalence#Normalization]
        ...
        while (($filename = readdir($handle)) !== FALSE) {
            if ($filename != '.' && $filename != '..') {
                if (@is_dir("$root$path/$filename")) {
                    $this->incrStats('metastore', 'num_dirs');
                    $this->fsckMetastore($root, "$path/$filename", $share);
                } else {
                    // Found a metafile
                    ...
                    // If file exists in landing zone, we already fsck-ed it in fsck() let's not repeat ourselves, shall we?
                    if (!file_exists("$local_path/$filename")) {
                        $this->fsckFile($local_path, $filename, FALSE, 'metastore', $share);
                    }
                }
            }
        }
    ...
    private static function resetDUStats($share, $full_path=null) {
    ...
    private static function updateSymlink($target, $symlink, $share, $file_path, $filename) {
        clearstatcache();
    ...
    private static function getCurrentId() {
        global $current_fsck_run;
        if (!empty($current_fsck_run)) {
            return $current_fsck_run->getId();
        }
        return 0;
    }

    public function log($type, $details) {
        static::slog($type, $details, $this->getId());
    }
    
    public static function slog($type, $details, $fsck_run_id = 0) {
        if (empty($fsck_run_id)) {
            $fsck_run_id = Fsck::getCurrentId();
        }
        $q = "INSERT INTO fsck_results SET fsck_run_id = :fsck_id, type = :type, details = :details";
        DB::insert($q, array('fsck_id' => $fsck_run_id, 'type' => $type, 'details' => $details));
    }

    private function incrStats($type, $subtype = NULL) {
        if (empty($subtype)) {
            @$this->stats[$type]++;
        } else {
            @$this->stats[$type][$subtype]++;
        }
    }

    private function getReport() {
        $start_time = strtotime($this->db_infos->started_at);
        $end_time = strtotime($this->db_infos->ended_at);

        $displayable_duration = duration_to_human($end_time - $start_time);

        $report = "fsck report
-----------
Scanned directory: " . $this->db_infos->dir . "

Started:  " . date('Y-m-d H:i:s', $start_time) . "
Ended:    " . date('Y-m-d H:i:s', $end_time) . "
Duration: $displayable_duration

Metadata Store:
  Found " . number_format($this->stats['metastore']['num_dirs']) . " directories
  Found " . number_format($this->stats['metastore']['num_files']) . " files

Landing Zone (shares):
  Found " . number_format($this->stats['landing_zone']['num_dirs']) . " directories
  Found " . number_format($this->stats['landing_zone']['num_files']) . " files
  Found " . number_format($this->stats['orphaned']['num_orphans']) . " orphans
...
Trash size:\n";
        ...
        $fsck_report = array(
            'temp_files' => array(),
            'wrong_file_size' => array(),
            'no_copies_found_files' => array(),
            'too_many_files' => array()
        );
        $logs = DB::getAll("SELECT * FROM fsck_results WHERE fsck_run_id = :fsck_id", $this->getId());
        foreach ($logs as $log) {
            if ($log->type == 'wrong_file_size') {
                $log->details = json_decode($log->details);
            }
            $fsck_report[$log->type][] = $log->details;
        }
        ...
        if (empty($fsck_report['no_copies_found_files']) && count($fsck_report['wrong_file_size']) == 0) {
            ...
            if (!empty($fsck_report['no_copies_found_files'])) {
                $fsck_report['no_copies_found_files'] = array_unique($fsck_report['no_copies_found_files']);
                sort($fsck_report['no_copies_found_files']);
                ...
                $report .= "    " . implode("\n    ", $fsck_report['no_copies_found_files']) . "\n\n";
            }
            ...
            if (count($fsck_report['wrong_file_size']) > 0) {
                ...
                foreach ($fsck_report['wrong_file_size'] as $info_array) {
                    $real_file_path = $info_array->file_path;
                    $file_size = $info_array->file_size;
                    $expected_file_size = $info_array->expected_file_size;
                    $original_file_path = $info_array->original_file_path;
                    $report .= "    $real_file_path is " . number_format($file_size) . " bytes; should be " . number_format($expected_file_size) . " bytes (as $original_file_path).\n";
                }
                $report .= "\n\n";
            }
        }
